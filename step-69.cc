...
namespace Step69
{
  ...
  //Workstream v1 copy but worse. We will generate copy/scratch data objects on demand rather than reusing.
  ...
  //Like v1 but we will now re-use scratch/copy data from a thread local list. 
  namespace taskflow_v2
  {
    ...
    void run(const Iterator &                         begin,
             ...
             const unsigned int queue_length = 2 * MultithreadInfo::n_threads(),
             const unsigned int chunk_size   = 8,
             const unsigned int total_cells = 0)
      ...
      struct ScratchAndCopyDataObjects
    {
      ...
      int max_work_tasks = 2*MultithreadInfo::n_threads();
      unsigned int idx = 0;
          ...
          auto worker_task = taskflow
                               .emplace([it = i,
                                ...
                                CopyData    *copy_data    = nullptr;
                                 ...
                                 auto &      copy    = copy_datas[idx];
                                 copy =
                                   std::make_unique<CopyData>(*copy_data);
  ...
  //Permits us to use a coloring of the mesh to allow scaling far beyond the above iterations
  namespace taskflow_v3
  {
    template <typename Worker,
              ...
              typename ScratchData,
              typename CopyData>
    void run(const std::vector<std::vector<Iterator>> &colored_iterators,
             Worker                                   worker,
             Copier                                   copier,
             const ScratchData &                      sample_scratch_data,
             const CopyData &                         sample_copy_data,
      ...
      if (MultithreadInfo::n_threads() == 1)
        {
          ...
          for (unsigned int color = 0; color < colored_iterators.size(); ++color)
            if (colored_iterators[color].size() > 0){
              for (auto i = colored_iterators[color].begin(); i != colored_iterators[color].end(); ++i)
                {
                  // need to check if the function is not the zero function. To
                  // check zero-ness, create a C++ function out of it and check that
                  if (static_cast<const std::function<void(
  ...
  public:
    ...
    void setup();
    void assemble(int timingcycle, std::string RunParam);
  ...
  template <int dim>
  void OfflineData<dim>::assemble(int timingcycle, std::string RunParam)
  {
...

    {
      TimerOutput::Scope scope(
        computing_timer,
        "offline_data - assemble lumped mass matrix, and c_ij " + std::to_string(timingcycle));
      ...
      const std::function<std::vector<types::global_dof_index>(const typename DoFHandler<dim>::cell_iterator &)> color_conflicts = [dofs_per_cell](const typename DoFHandler<dim>::cell_iterator &CellIterator) -> std::vector<types::global_dof_index> {
        std::vector<types::global_dof_index> conflicting_dofs(dofs_per_cell);
        CellIterator->get_dof_indices(conflicting_dofs);
        return conflicting_dofs;
      };
    if (RunParam == "WorkStream_No_Color"){
      WorkStream::run(dof_handler.begin_active(),
                      dof_handler.end(),
                      local_assemble_system,
                      copy_local_to_global,
                      scratch_data,
                      CopyData<dim>(), 2 * MultithreadInfo::n_threads(), 1);
    } else if (RunParam == "WorkStream_Color"){
      WorkStream::run(GraphColoring::make_graph_coloring(dof_handler.begin_active(),
                                dof_handler.end(),
                                color_conflicts),
                      local_assemble_system,
                      copy_local_to_global,
      ...
      taskflow_v2::run(dof_handler.begin_active(),
                              ...
                              scratch_data,
                            CopyData<dim>());
    } else if (RunParam == "Taskflow_V3"){
      taskflow_v3::run(GraphColoring::make_graph_coloring(dof_handler.begin_active(),
                              dof_handler.end(),
                              color_conflicts),
                              local_assemble_system,
                              copy_local_to_global,
  ...
  template <int dim>
  void MainLoop<dim>::run(std::string RunParam)
  {
    ...
    pcout << "Running with " << RunParam << std::endl;
    //pcout << "done" << std::endl;
    ...
    int max_timingcycle = 10;
    {
      ...
      offline_data.setup();
      for (int i = 0; i<max_timingcycle; i++, timingcycle++)
      offline_data.assemble(timingcycle, RunParam);
    ...
    */
    ...
    TimerOutput::OutputData optdata = TimerOutput::OutputData::total_wall_time;
    
    std::map< std::string, double > timer_summary = computing_timer.get_summary_data(TimerOutput::OutputData::total_wall_time);
    float timings[max_timingcycle];
    int lazy_solution_timer = 0;
    for (auto section : timer_summary)
    {
      if ((lazy_solution_timer > 0) and (lazy_solution_timer < max_timingcycle+1)){
      timings[lazy_solution_timer-1] = section.second;
      }
      lazy_solution_timer += 1;
    }
    float mean = 0;
    float sum = 0;
    for (int i = 0; i <max_timingcycle ; i++){
      sum += timings[i];
    }
    mean = sum/max_timingcycle;
    float var = 0;
    for(int i = 0; i <max_timingcycle ; i++)
    {
      var += (timings[i] - mean) * (timings[i] - mean);
    }
    var /= max_timingcycle + 1;
    float sd = sqrt(var);
    pcout << "Mean: " << std::to_string(mean) << " Stdev: " << std::to_string(sd) << std::endl;
  }
  ...
  try
    {
      ...
      Utilities::MPI::MPI_InitFinalize mpi_initialization(argc, argv, Cores);

      MPI_Comm      mpi_communicator(MPI_COMM_WORLD);
      MainLoop<dim> main_loop(mpi_communicator);

      //std::cout << "Running with " << MultithreadInfo::n_threads() << " threads per process" << std::endl;
      if (not (RunParam == "Silent")){
        main_loop.run(RunParam);
      }
